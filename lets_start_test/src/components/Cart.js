...
const Cart = () => {
  ...
  return (
    <div className="w-6/12 m-auto">
      ...
      <div
        className="bg-red-400 p-3 rounded-lg w-3/12 m-auto text-center mb-4 cursor-pointer"
        onClick={handleClearCart}
      >
        <button className="text-white font-bold" data-testid="clear-button">
          Clear cart
        </button>
      </div>
      <div className="p-2 mx-2">
        {cartItem?.map((item, index) => (
          <RestaurantItem data={item} key={index} />
        ))}
      </div>
    </div>
  );
